...
import { NewFeature, FeatureStatus, Feature } from '@rothenbergt/backstage-plugin-roadmap-common';
  ...
  return useMutation(
      ...
      api.updateFeatureStatus(id, status),
    {
      onSuccess: (updatedFeature) => {
        ...
        queryClient.setQueryData<FeatureWithVote[]>(['roadmap', 'features'], (oldData) => {
          if (!oldData) return [];
